...
function Row(props: LinkTableBodyProps) {
        ...
        {props.keys.map((header: any, index: any) => {
          return (
            <React.Fragment key={index}>
              <td className={`table__body table__body-cell table__column-${index}`}>
                {props.linkedColumn === header.key ? (
                  <Link className="link" to={`/researchers/${props.data['_id']}`}>
                    {header.key === 'user_lastname'
                      ? `${props.data[header.key]} ${props.data['user_name']}`
...
function TableBody(props: LinkTableBodyProps) {
  return (
    <>
      ...
      <tr>
        ...
        {props.headers.map((header, index) => {
          return (
            ...
            >
              {header.key === 'user_lastname' ? t('table.full_name') : t(`table.${header.label}`)}
              <TableSortingArrows
                order={props.order}
...
export default function CustomLinkTable(props: LinkTableProps) {
      ...
      <table className="table">
        ...
        <tbody>
          <TableBody
            ...
            linkedColumn={props.linksColumn}
            loading={props.loading}
          />
          ...
          {emptyRows > 0 && ( /** replace props.data.length with emptyRows */
            <tr>
              {props.loading ? (
                ...
                <td className="table__empty" colSpan={5}>
                  {props.data.length === 0 ? <TableEmptyImg message={t('table.noData')} /> : null}
                </td>
              )}
            </tr>
          )}
